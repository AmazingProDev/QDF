...
function findStructure(rows: unknown[][]): Structure {
  ...
  header.forEach((label, c) => { const band = bandAtColumn[c]; if (!band) return; if ((label.includes("debit") || label.includes("debit 4g") || label.includes("throughput")) && (label.includes("dl") || label.includes("4g"))) bands[band].throughput = c; if (label.includes("prb") && (label.includes("dl") || label.includes("bh") || label === "prb")) bands[band].prb = c; if (label.includes("traffic") || label.includes("trafic")) bands[band].traffic = c; if (label.includes("avail")) bands[band].availability = c; if (label === "ta" || label.includes("timing advance")) bands[band].ta = c; });
