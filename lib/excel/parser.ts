...
// Do not reverse-match a candidate against a header: empty separator cells in
// banded KPI layouts would otherwise match every requested column.
const headerMatches = (header: string, candidates: string[]) => Boolean(header) && candidates.some((candidate) => header === candidate || header.includes(candidate));
...
type Structure = { headerRow: number; columns: Record<string, number | undefined>; bands: Record<Band, { throughput?: number; prb?: number; traffic?: number; availability?: number; ta?: number }>; format: "qualification" | "kpi-snapshot" };
function findStructure(rows: unknown[][]): Structure {
  ...
  header.forEach((label, c) => { const band = bandAtColumn[c]; if (!band) return; if ((label.includes("debit") || label.includes("debit 4g") || label.includes("throughput")) && (label.includes("dl") || label.includes("4g"))) bands[band].throughput = c; if (label.includes("prb") && (label.includes("dl") || label.includes("bh") || label === "prb")) bands[band].prb = c; if (label.includes("traffic") || label.includes("trafic") || /(^|\s)traf(\s|$)/.test(label)) bands[band].traffic = c; if (label.includes("avail")) bands[band].availability = c; if (label === "ta" || label.includes("timing advance")) bands[band].ta = c; });
  const availableBands = BANDS.filter((band) => bands[band].throughput !== undefined || bands[band].prb !== undefined);
  if (!availableBands.length) throw new Error("Aucun KPI Débit 4G DL ou Usage_PRB_DL_BH n’est détecté. Vérifiez les en-têtes par bande.");
  const columns = { situation: index("situation", "status"), sector: index("secteurs", "secteur"), hetsite: index("hetsite"), dr: index("dr"), plaque: index("plaque"), vendor: index("vendor", "fournisseur"), responsibility: index("responsabilite"), action: index("analyse/action", "analyse action", "action deploiement", "action", "constat"), qualif: index("qualif", "qualification"), degradedBands: index("bandes debit degrade", "bandes debit degradees"), chargedBands: index("bandes chargees"), prodOptim: index("prod optim"), prodMaintenance: index("prod maintenance"), prodDeployment: index("prod deploiement"), prodEngineering: index("prod ingenierie") };
  const format = columns.degradedBands !== undefined || columns.chargedBands !== undefined || columns.qualif !== undefined ? "qualification" : "kpi-snapshot";
  return { headerRow, columns, bands, format };
}
...
export async function parseTelecomWorkbook(file: File): Promise<ParseResult> {
  ...
  const worksheet = workbook.worksheets.find((sheet) => normalized(sheet.name) === "analyse") || workbook.worksheets.find((sheet) => { for (let r = 1; r <= Math.min(30, sheet.rowCount); r++) { const cells = sheet.getRow(r).values; if (Array.isArray(cells) && cells.some((cell) => ["secteurs", "secteur"].includes(normalized(toCellText(cell))))) return true; } return false; });
  ...
  for (let r = structure.headerRow + 2; r <= lastRow; r++) {
    ...
    const read = (key: string) => toCellText(valueAt(row, structure.columns[key])); const rawResponsibility = read("responsibility"); const snapshot: SectorSnapshot = { sector, originalSector, situation: read("situation") || undefined, hetsite: read("hetsite") || undefined, dr: read("dr") || undefined, plaque: read("plaque") || undefined, vendor: read("vendor") || undefined, responsibility: rawResponsibility ? qualificationResponsibility(rawResponsibility) : undefined, action: read("action") || undefined, qualif: read("qualif") || undefined, degradedBands: bandList(valueAt(row, structure.columns.degradedBands)), chargedBands: bandList(valueAt(row, structure.columns.chargedBands)), prodOptim: dateValue(valueAt(row, structure.columns.prodOptim)), prodMaintenance: dateValue(valueAt(row, structure.columns.prodMaintenance)), prodDeployment: dateValue(valueAt(row, structure.columns.prodDeployment)), prodEngineering: dateValue(valueAt(row, structure.columns.prodEngineering)), bands: {} as SectorSnapshot["bands"] };
    for (const band of BANDS) { const cols = structure.bands[band]; snapshot.bands[band] = { throughput: asNumber(valueAt(row, cols.throughput)), prb: asNumber(valueAt(row, cols.prb)), traffic: asNumber(valueAt(row, cols.traffic)), availability: asNumber(valueAt(row, cols.availability)), ta: asNumber(valueAt(row, cols.ta)) }; } allRecords.push(snapshot); if (records.has(sector)) duplicateSet.add(sector); else records.set(sector, snapshot);
  }
  return { records, allRecords, duplicates: [...duplicateSet], rowCount, sheetName: worksheet.name, format: structure.format, detectedBands: Object.fromEntries(BANDS.map((band) => [band, { throughput: structure.bands[band].throughput !== undefined, prb: structure.bands[band].prb !== undefined }])) as ParseResult["detectedBands"] };
}
