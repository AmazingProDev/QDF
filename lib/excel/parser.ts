...
export async function parseTelecomWorkbook(file: File): Promise<ParseResult> {
  ...
  const structure = findStructure(topRows); const records = new Map<string, SectorSnapshot>(); const duplicateSet = new Set<string>(); let rowCount = 0, emptyRun = 0;
  ...
  for (let r = structure.headerRow + 2; r <= lastRow; r++) {
    ...
    if (records.has(sector)) { duplicateSet.add(sector); continue; }
    ...
    for (const band of BANDS) { const cols = structure.bands[band]; snapshot.bands[band] = { throughput: asNumber(valueAt(row, cols.throughput)), prb: asNumber(valueAt(row, cols.prb)), availability: asNumber(valueAt(row, cols.availability)), ta: asNumber(valueAt(row, cols.ta)) }; } records.set(sector, snapshot);
  }
  return { records, duplicates: [...duplicateSet], rowCount, sheetName: worksheet.name, detectedBands: Object.fromEntries(BANDS.map((band) => [band, { throughput: structure.bands[band].throughput !== undefined, prb: structure.bands[band].prb !== undefined }])) as ParseResult["detectedBands"] };
}
