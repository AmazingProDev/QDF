...
describe("Excel export", () => {
  it("keeps Analyse formatting, appends impact columns, and adds the operational summary", async () => {
    ...
    expect(exportedAnalyse?.getCell(1, 14).value).toBe("IEA-R Résorption");
    expect(exportedAnalyse?.getCell(2, 14).value).toBe(.68);
    expect(exportedAnalyse?.getCell(1, 16).value).toBe("IEA-E Évolution");
    expect(exportedAnalyse?.getCell(1, 25).value).toBe("L1800 DL Avant");
    expect(exportedAnalyse?.getCell(2, 25).value).toBe(6);
    expect(exportedAnalyse?.getCell(1, 69).value).toBe("Source KPI BEFORE");
    expect(exportedAnalyse?.getCell(2, 69).value).toBe("Référence Ookla");
    expect(exportedAnalyse?.getCell(1, 70).value).toBe("L1800 Traffic référence [MByte]");
    expect(exportedAnalyse?.getCell(2, 70).value).toBe(202337.23);
    expect(summary?.getCell("A1").value).toContain("Synthèse opérationnelle");
    expect(summary?.getCell("A4").value).toBe("Optim");
    expect(summary?.getColumn(1).values).toContain("Contribution IEA-E par responsabilité");
  });
});
