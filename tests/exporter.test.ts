...
describe("Excel export", () => {
  it("keeps Analyse formatting, appends impact columns, and adds the operational summary", async () => {
    ...
    expect(exportedAnalyse?.getCell(1, 43).value).toBe("Source KPI BEFORE");
    expect(exportedAnalyse?.getCell(2, 43).value).toBe("Référence Ookla");
    expect(exportedAnalyse?.getCell(1, 44).value).toBe("L1800 Traffic référence [MByte]");
    expect(exportedAnalyse?.getCell(2, 44).value).toBe(202337.23);
