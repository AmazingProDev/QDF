...
export function UploadPanel({ label, description, file, onFile }: { label: "BEFORE" | "AFTER"; description: string; file?: File; onFile: (file?: File) => void }) {
  ...
  return <section className={`upload-card ${dragging ? "is-dragging" : ""}`} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); accept(e.dataTransfer.files[0]); }}>
    <div className="upload-label">{label}</div><h2>{label === "BEFORE" ? "Référence initiale" : "Qualification finale"}</h2><p>{description}</p>
